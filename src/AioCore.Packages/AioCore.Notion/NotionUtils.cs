...
namespace AioCore.Notion
{
    public static class NotionUtils
    {
        ...
        private static readonly Regex TrimSpaces = new(@"[\s-:()/\\\W]+", RegexOptions.None);
        //sentence = regex.Replace(sentence, " ");

        public static Uri GetPageUri(Guid pageId, string title)
            => new(new Uri(Constants.BaseUrl), $"{Uri.EscapeDataString(TrimSpaces.Replace(title ?? "", "-"))}-{pageId:N}");
