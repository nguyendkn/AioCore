    ...
    public static class HtmlBlockExtensions
    {
        ...
        public static string GetHtmlAbstract(this RecordMap recordMap, Guid pageId = default)
            => recordMap.GetHtml(pageId, throwIfBlockMissing: false, stopBeforeFirstSubHeader: true,
                throwIfCantDecodeTextData: false);
        ...
        /// <returns></returns>
        public static string GetHtml(this RecordMap recordMap, Guid pageId = default, int maxBlocks = 0,
            bool throwIfBlockMissing = true, bool stopBeforeFirstSubHeader = false,
            bool throwIfCantDecodeTextData = true)
        {
...
#if !DEBUG
                AcceptedBlockTypes =
 new List<string> { "text", "header", "sub_header", "sub_sub_header", "bulleted_list", "image", "quote", "column_list", "column", "callout" },
#endif
                ...
                {
                    if (data != null)
                        sb.Append("<div class=\"notion-image-block\">").AppendImage(data, block.Id)
                            .AppendLine("</div>");
                    ...
                    return (true,
                        StartColumn: (columnIndex, column) =>
                        {
                            sb.Append(FormattableString.Invariant(
                                $"<div class=\"notion-column\" style=\"width: calc((100% - {46 * (totalColumns - 1)}px) * {column.Ratio});\">"));
                            return () => sb.Append("</div>");
                        },
                        TransformColumnSeparator: columnIndex =>
                            sb.Append(
                                "<div class=\"notion-column-separator\"><div class=\"notion-column-separator-line\"></div></div>"),
                ...
                TransformCallout = (data, block) =>
                {
                    ...
                    if (Uri.TryCreate(data.Format.PageIcon, UriKind.Absolute, out var iconUrl))
                        ...
                        sb.AppendLine(
                            @$"<img class=""notion-emoji"" src=""{GetTwitterEmojiUrl(data.Format.PageIcon)}"" />");

                    options.TransformText(data.Text, block);
                    sb.AppendLine("</div>");
                    return true;
                },
#if DEBUG
                TransformOther = block => true
#endif
            };
        ...
        private static StringBuilder AppendText(this StringBuilder sb, BlockTextData? data)
        {
            ...
            foreach (var line in data.Lines.Where(l => l != null))
            {
                ...
                if (tag != null)
                {
                    ...
                    if (line.HyperlinkUrl != null)
                        sb.Append(" href=\"").Append(Uri.EscapeDataString(line.HyperlinkUrl)).Append('"');
        ...
        private static StringBuilder AppendImage(this StringBuilder sb, BlockImageData? data, Guid blockId)
        {
            if (data == null) return sb;
            var imageUrl = $"{data.ImageUrl}?table=block&id={blockId:D}&cache=v2";
            if (data.Format != null)
                sb.Append("<img style=\"width:").Append(data.Format.BlockWidth).Append("px\" src=\"").Append(imageUrl)
                    .Append("\"/>");
            else
                sb.Append("<img src=\"").Append(imageUrl).Append("\"/>");

            if (!string.IsNullOrWhiteSpace(data.Caption))
                sb.Append("<div class=\"notion-text-block notion-image-caption\">").Append(data.Caption)
                    .Append("</div>");

            return sb;
        }

        private static StringBuilder? GetTwitterEmojiUrl(this string emojiString)
        {
